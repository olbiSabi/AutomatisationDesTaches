...
FILEAPP=/home/talhent/production/LOGAPP
# Variable contenant le chemin du rendu LOGAPP
RENDU=/home/talhent/production/renduAPP.html
...
do
	...
	then
	 ...
	 echo "<tr><td>" $(echo $FILE | awk -F . '{print $1"."$2}')" </td><td> `grep $Fichier $ADERHPARAM | sed -n 2p | cut -d/ -f 2`</td><td>$HEURE_DEBUT</td><td>$HEURE_FIN</td>
